...
public class TLMediaMovieBuilder {
    ...
    private static String DIR_NAME = "GocciCamera";
...
//**********************************************************************
     ...
     */
    public static final File getCaptureFile(final String type, String awsPostName, final String ext) {
        final File dir = new File(Environment.getExternalStoragePublicDirectory(type), DIR_NAME);
        Log.d(TAG, "path=" + dir.toString());
        dir.mkdirs();
        if (dir.canWrite()) {
            return new File(dir, awsPostName + ext);
        }
        return null;
    }
    ...
    private static final class MuxerTask extends Thread {
        ...
        private final String mAwsStamp;
        ...
        public MuxerTask(final TLMediaMovieBuilder builder, final TLMediaMovieBuilderCallback callback) {
            ...
            mAwsStamp = builder.mAwsPostName;
        }
                    ...
                    try {
                        ...
                        if (videoIn != null) {
                            ...
                            if (format != null) {
                                ...
                                if (DEBUG)
                                    Log.v(TAG, "found video data:format=" + format + "track=" + videoTrack);
                            }
                        }
                        ...
                        if (audioIn != null) {
                            ...
                            if (format != null) {
                                ...
                                if (DEBUG)
                                    Log.v(TAG, "found audio data:format=" + format + "track=" + audioTrack);
                            }
                        }
                            ...
                            for (; mIsRunning && ((videoTrack >= 0) || (audioTrack >= 0)); ) {
                                if (videoTrack >= 0) {
                                    try {
                                        ...
                                        if (DEBUG)
                                            Log.d(TAG, String.format("MuxerTask(video):size=%d,presentationTimeUs=%d,",
                                ...
                                if (audioTrack >= 0) {
                                    try {
                                        ...
                                        if (DEBUG)
                                            Log.d(TAG, String.format("MuxerTask(audio):size=%d,presentationTimeUs=%d,",
            ...
            synchronized (mSync) {
                if (mCallback != null) {
                    mCallback.onFinished(mIsRunning && isMuxerStarted ? mMuxerFilePath : null,
                            mIsRunning && isMuxerStarted ? mAwsStamp : null);
                }
            }
        }
    }
